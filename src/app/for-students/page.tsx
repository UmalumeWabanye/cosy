...
const BENEFITS = [
  { icon: <FilterListRoundedIcon sx={{ fontSize: 28, color: '#1976d2' }} />, title: 'Smart Filters', desc: 'Search by campus, budget, room type, and NSFAS eligibility. Find exactly what you need.' },
  { icon: <VerifiedRoundedIcon sx={{ fontSize: 28, color: '#1976d2' }} />, title: 'Verified Listings', desc: 'Every property is reviewed. No fake listings, no hidden fees, no surprises.' },
  { icon: <ChatBubbleRoundedIcon sx={{ fontSize: 28, color: '#1976d2' }} />, title: 'Direct Messaging', desc: 'Message landlords securely through Cosy. Share contact info only when you\'re ready.' },
  { icon: <TrendingUpRoundedIcon sx={{ fontSize: 28, color: '#1976d2' }} />, title: 'Application Tracking', desc: 'Track your application status in real-time. Know exactly where you stand.' },
];

const PROCESS_STEPS = [
  { num: '1', title: 'Search Near Campus', desc: 'Filter by your university, budget, room type, or NSFAS eligibility to find the perfect match in seconds.' },
  { num: '2', title: 'View Verified Listings', desc: 'Browse real photos, honest pricing, and landlord ratings. Compare your top options side by side.' },
  { num: '3', title: 'Apply in Minutes', desc: 'Send a rental request directly to the landlord. No forms. No waiting. Just a simple application.' },
  { num: '4', title: 'Move In With Confidence', desc: 'All landlords are identity-verified. See exactly what you\'re getting before you commit to anything.' },
];

const FEATURES = [
  { icon: <SchoolRoundedIcon sx={{ fontSize: 28, color: '#1976d2' }} />, title: 'Campus-First Search', desc: 'Filter by walking distance to campus, proximity to libraries, or transport hubs.' },
  { icon: <AccountBalanceRoundedIcon sx={{ fontSize: 28, color: '#1976d2' }} />, title: 'NSFAS Friendly', desc: 'Filter specifically for NSFAS-accredited properties. No more being turned away.' },
  { icon: <SecurityRoundedIcon sx={{ fontSize: 28, color: '#1976d2' }} />, title: 'Privacy Protected', desc: 'Your contact details are only shared once you choose to apply to a listing.' },
];
    ...
    <ThemeProvider theme={theme}>
      ...
      <Box sx={{ position: 'relative', height: { xs: 420, sm: 480, md: 540 }, overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <Box sx={{ position: 'relative', zIndex: 2, textAlign: 'center', px: 2, width: '100%', maxWidth: 780, mx: 'auto' }}>
          <Typography sx={{ color: '#1976d2', fontWeight: 700, letterSpacing: 1.2, textTransform: 'uppercase', fontSize: 12, mb: 2 }}>Find Your Home</Typography>
          <Typography component="h1" sx={{ fontWeight: 700, fontSize: { xs: '2rem', sm: '2.8rem', md: '3.4rem' }, lineHeight: 1.1, letterSpacing: '-0.02em', color: '#222222', mb: 1.5 }}>
            Student housing made simple and stress-free
          </Typography>
          <Typography sx={{ color: '#717171', fontSize: { xs: 15, md: 17 }, fontWeight: 400, mb: 4, lineHeight: 1.65, maxWidth: 600, mx: 'auto' }}>
            Browse verified accommodation near your campus with smart filters for budget, room type, and NSFAS eligibility. Apply in minutes. Move in with confidence.
          </Typography>
          <Box sx={{ display: 'flex', gap: 2, justifyContent: 'center', flexWrap: 'wrap' }}>
            <Button component={Link} href="/browse?source=for-students" variant="contained"
            ...
            <Button component={Link} href="/register?source=for-students" variant="outlined"
      ...
      <Box sx={{ bgcolor: '#fff', py: { xs: 8, md: 12 } }}>
        <Container maxWidth="lg">
          <Box sx={{ textAlign: 'center', mb: 8 }}>
            <Typography sx={{ fontSize: { xs: 22, md: 28 }, fontWeight: 700, color: '#222222', letterSpacing: '-0.02em' }}>Why students choose Cosy</Typography>
            <Typography sx={{ color: '#717171', mt: 1, fontSize: 15 }}>Everything you need to find and secure your perfect student home.</Typography>
          </Box>
      ...
      <Box sx={{ bgcolor: '#f8f9fa', py: { xs: 8, md: 12 } }}>
        <Container maxWidth="lg">
          <Box sx={{ textAlign: 'center', mb: 8 }}>
            <Typography sx={{ fontSize: { xs: 22, md: 28 }, fontWeight: 700, color: '#222222', letterSpacing: '-0.02em' }}>Your journey to finding home</Typography>
            <Typography sx={{ color: '#717171', mt: 1, fontSize: 15 }}>Four simple steps from search to move-in.</Typography>
          </Box>
        ...
        <Container maxWidth="lg">
          <Grid container spacing={{ xs: 4, md: 8 }} sx={{ alignItems: 'center' }}>
            <Grid size={{ xs: 12, md: 6 }}>
              <Box>
                <Typography sx={{ color: '#1976d2', fontWeight: 700, letterSpacing: 1.2, textTransform: 'uppercase', fontSize: 12, mb: 2 }}>Search With Confidence</Typography>
                <Typography variant="h2" sx={{ fontWeight: 800, fontSize: { xs: '1.9rem', md: '2.6rem' }, color: '#222222', lineHeight: 1.2, mb: 3 }}>
                  Find accommodation that fits you perfectly
                </Typography>
                <Typography sx={{ color: '#717171', fontSize: 15, lineHeight: 1.8, mb: 4 }}>
                  Whether you're on NSFAS, a bursary, or paying privately — Cosy helps you find verified, affordable accommodation near your campus with just a few clicks.
                </Typography>
                <Button component={Link} href="/browse?source=for-students-features" variant="contained"
      ...
      <Box sx={{ background: 'linear-gradient(135deg, #021a38 0%, #0c3f73 50%, #1565c0 100%)', py: { xs: 8, md: 12 }, px: 2, textAlign: 'center', position: 'relative', overflow: 'hidden', '&::before': { content: '""', position: 'absolute', inset: 0, background: 'radial-gradient(circle at 80% -10%, rgba(147,214,255,0.22), transparent 50%)' } }}>
        <Container maxWidth="sm" sx={{ position: 'relative', zIndex: 1 }}>
          ...
          <Typography sx={{ color: 'rgba(255,255,255,0.8)', mb: 5, fontSize: 15, lineHeight: 1.65 }}>
            Join over 50,000 South African students who\'ve found their perfect accommodation with Cosy. Searching is completely free.
          </Typography>
          <Box sx={{ display: 'flex', gap: 2, justifyContent: 'center', flexWrap: 'wrap' }}>
            <Button component={Link} href="/browse?source=for-students-cta" variant="contained"
            ...
            <Button component={Link} href="/register?source=for-students-cta"
        ...
        <Container maxWidth="lg">
          <Grid container spacing={4} sx={{ mb: 4 }}>
            <Grid size={{ xs: 12, sm: 6, md: 3 }}>
              ...
              <Stack spacing={1}>
                {['About', 'For Landlords', 'Contact'].map(link => (
                  <Link key={link} href={link === 'For Landlords' ? '/for-landlords' : '#'} style={{ color: 'rgba(255,255,255,0.65)', textDecoration: 'none', fontSize: 13, transition: 'color 0.2s', cursor: 'pointer' }}>
